...
const baseUrl = "https://toutenmel.fr";

function ShareButtons({ title, category }: { title: string; category: string }) {
  const text = `Découvrez "${title}" (${category}) par Mel — Artiste peintre`;
  const url = baseUrl;
    ...
    <div className="flex items-center gap-1">
      {shareLinks.map((link) => (
        <a
          ...
          className={`p-2 rounded-full text-warm-gray/40 ${link.color} transition-colors`}
        >
          {link.icon}
        </a>
      ))}
